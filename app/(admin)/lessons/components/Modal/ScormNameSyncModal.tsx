    ...
    return <Modal open={open} onCancel={handleClose} width={1100} title="Đồng bộ dữ liệu bài giảng từ Google Sheets"
        ...
        {sheetsLoading && !sheets.length ? <Spin /> : <>
            ...
            {mode === "course-mappings" && hasPreview && !mappingProgress && <>
                ...
                <Table size="small" rowKey={(row) => `${row.programCode}-${row.lessonId}`} pagination={false} scroll={{ x: 1230, y: 500 }} dataSource={mappingPreviews.flatMap((group) => group.preview.rows.map((row) => ({ ...row, programCode: group.programCode, sourceSheet: group.sheetName })))} columns={[
                    ...
                    { title: "Mapping từ Sheet", width: 180, render: (_, row) => row.sheetMappings.length ? <Space direction="vertical" size={2}>{row.sheetMappings.map((item) => <Typography.Text key={`${item.packageId}-${item.courseId}`}>{item.packageId} / {item.courseId}</Typography.Text>)}</Space> : "—" },
